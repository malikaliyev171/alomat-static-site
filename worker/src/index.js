    ...
    await env.DB.prepare(
      `INSERT INTO signals (
       ...
       ON CONFLICT(external_id) DO UPDATE SET
         ...
         title_en = CASE WHEN ? = 1 THEN excluded.title_en ELSE signals.title_en END,
         summary_en_json = CASE WHEN ? = 1 THEN excluded.summary_en_json ELSE signals.summary_en_json END,
         rich_summary_en_json = CASE WHEN ? = 1 THEN excluded.rich_summary_en_json ELSE signals.rich_summary_en_json END,
         title_tr = CASE WHEN ? = 1 THEN excluded.title_tr ELSE signals.title_tr END,
         summary_tr_json = CASE WHEN ? = 1 THEN excluded.summary_tr_json ELSE signals.summary_tr_json END,
         rich_summary_tr_json = CASE WHEN ? = 1 THEN excluded.rich_summary_tr_json ELSE signals.rich_summary_tr_json END,
      ...
      .bind(
        ...
        signal.language,
        signal.created_at,
        Number(signal.has_title_en),
        Number(signal.has_summary_en),
        Number(signal.has_rich_summary_en),
        Number(signal.has_title_tr),
        Number(signal.has_summary_tr),
        Number(signal.has_rich_summary_tr),
      )
      .run();
  } catch {
