...
const labelMap = {
  uz: {
    ...
    codeSent: "Kod yuborildi. E-pochtangizni tekshiring.",
    requestSubmit: "Havola va kod yuborish",
    verifySubmit: "Kodni tasdiqlash",
    invalidEmail: "Iltimos, to'g'ri e-pochta kiriting.",
    invalidCode: "6 xonali kodni kiriting.",
    authError: "Hozir kodni tekshirib bo'lmadi. Qayta urinib ko'ring.",
  },
  en: {
    palette: "Palette",
    save: "Saved",
    ...
    requestSubmit: "Send link and code",
    verifySubmit: "Verify code",
    invalidEmail: "Please enter a valid email.",
    invalidCode: "Enter the 6 digit code.",
    authError: "We could not verify the code right now. Try again.",
  },
};
...
function initNameForm() {
  ...
  nameForm?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (!pendingAuthEmail || nameCodeInput?.hidden || pendingAuthEmail !== email) {
        await postAuthJson("/api/auth/request-code", { email });
        pendingAuthEmail = email;
        if (nameCodeInput) {
          nameCodeInput.hidden = false;
          nameCodeInput.focus?.({ preventScroll: true });
        }
        if (nameCodeField) {
          nameCodeField.hidden = false;
        }
        if (nameSubmitLabel) {
          nameSubmitLabel.textContent = labelMap[locale].verifySubmit;
        }
        setNameAuthStatus(labelMap[locale].codeSent, "success");
        return;
      }

      const code = String(nameCodeInput?.value ?? "").trim();
      if (!/^\d{6}$/.test(code)) {
        setNameAuthStatus(labelMap[locale].invalidCode, "error");
        return;
      }

      const verified = await postAuthJson("/api/auth/verify-code", { email: pendingAuthEmail, code });
      const verifiedEmail = normalizeAuthEmail(verified.email || pendingAuthEmail).toLowerCase();
      storage?.setItem(storageKeys.name, verifiedEmail);
      storage?.removeItem(legacyStorageKeys.name);
      updateNameDisplay(verifiedEmail);
      if (nameDisplay) {
        nameDisplay.textContent = labelMap[locale].greeting(verifiedEmail);
      }
      if (nameInput) {
        nameInput.value = verifiedEmail;
        nameInput.blur();
      }
  ...
  nameInput?.addEventListener("input", () => {
    ...
    if (pendingAuthEmail && normalizeAuthEmail(nameInput.value) !== pendingAuthEmail) {
      resetNameAuthCodeStep();
    }
  });
}
