...
const labelMap = {
  uz: {
    ...
    emailFailed: "Mail yuborilmadi. Qayta urinib ko'ring.",
  },
  en: {
    palette: "Palette",
    save: "Saved",
    ...
    emailFailed: "The email could not be sent. Please try again.",
  },
};
...
async function sendWelcomeEmail(email) {
  const response = await fetch(buildApiUrl("/api/auth/welcome-email"), {
    method: "POST",
...
function initNameForm() {
  ...
  nameForm?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      setNameAuthStatus(labelMap[locale].emailFailed, "error");
      return;
    }
